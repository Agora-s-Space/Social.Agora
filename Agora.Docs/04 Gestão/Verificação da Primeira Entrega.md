...
- **Estratégia de ramificação, convenção de commit e critérios de merge:** GitHub Flow simplificado com `main` + branches curtas (`feat/`, `docs/`, `chore/`, `fix/`); commits seguem **Conventional Commits**; merge exige PR ≥ 1 aprovação + CI verde + sem conflitos — detalhado em [[04 Gestão/Operações e Deploy#9. Estratégia de ramificação e revisão|Operações e Deploy §9]], coerente com o fluxo de promoção dev → staging → produção ([[03 Decisões/ADR-004 Ambientes|ADR-004]]).
